...
\title{面向MZM任意点偏压控制的精确仿射建模与全周期辨识}

% 投稿前仅需替换本块中的真实作者姓名与通信邮箱；科学正文已完成。
\author{第一作者，第二作者%
\thanks{作者单位：西安电子科技大学综合业务网理论及关键技术国家重点实验室，西安~710071（通信作者电子邮箱：author@xidian.edu.cn）。}}

\markboth{面向~MZM 任意点偏压控制的精确仿射建模与全周期辨识}%
{作者~\MakeLowercase{\textit{等}}：MZM 任意点偏压的仿射建模与辨识}

\maketitle

\begin{abstract}
马赫--曾德尔调制器（MZM）的任意点偏压控制需要在整个传递周期内保持可辨识的相位读出，而单谐波或谐波比值会引入灵敏度零点、分支歧义和通道标定误差。本文证明，在单余弦功率传递、测量窗内参数冻结以及线性接收--解调条件下，二维谐波观测是偏置相位单位圆的精确仿射像。增益失配、参考相位误差、导频同步电串扰和偏置无关的有限消光比统一表现为通道矩阵与偏置向量；当通道矩阵满秩且辅助信息唯一确定椭圆的方向和相位原点时，椭圆辨识与闭式反演给出全周期偏置相位。本文进一步建立标量读出的局部静差、条件噪声传播和重标定判据，并给出目标点无关的积分控制律。数值实验验证了仿射模型的精确闭合、全周期反演和一致闭环增益。真实硬件扫描中，监督式与纯椭圆路径的五折交错留出~rms 分别为~53.7 和~61.7~mrad；16 个目标点的静态响应斜率为~1.052，决定系数为~0.986。结果表明，二维仿射建模能够在真实接收链中辨识全周期相位，并把任意点偏压控制转化为可标定的相位反演问题。
\end{abstract}
...
铌酸锂（LiNbO$_3$）、磷化铟（InP）以及薄膜铌酸锂马赫--曾德尔调制器（MZM）的工作点会随温度、电荷积累和光折变效应缓慢漂移~\cite{salvestrini2011,chen2011,wang2018,wang2022}。小幅低频导频（dither）能够在不中断业务信号的条件下持续观测这种漂移：探测光电流的一次谐波在零点/峰值点附近过零，二次谐波在正交点附近过零，由此形成闭环误差信号。面向开关键控（OOK）、IQ 调制器和偏振复用发射机的特殊点锁定及其多环扩展已经形成成熟的实现路线~\cite{cho2006,sotoodeh2011,kawakami2012,fu2013,li2017}。

微波光子移相器与矢量调制器~\cite{yao2009,capmany2007}、单边带（SSB）权重控制~\cite{izutsu1981}、模拟光计算~\cite{shen2017} 和啁啾管理还要求偏置稳定在传递曲线的连续设定点上。现有任意点方法包括单~MZM 的预标定谐波比值~\cite{wang2010}、平均功率导数或局部差分~\cite{tao2014,yuan2019}、面向具体链路的模型控制~\cite{teng2023}，以及~IQ 调制器中的专用抖动序列与相关检测~\cite{li2018,peng2025}。模型优化已用于单~MZM 的零点和正交点控制~\cite{svarny2022,weller2025}；DLA2C 则以直流、H1 和导频相关乘积构造多维特征，通过神经网络与粒子群搜索实现任意点控制~\cite{hu2026dla2c}。这些方法分别利用标定比例、局部斜率、专用激励或在线优化建立偏置状态与观测特征之间的对应关系。

连续设定点控制的核心是从完整传递周期中恢复偏置相位，同时把接收链的增益、偏置和通道混合与相位本身分离。单个光滑周期标量不可能在全周期上单射，并在临界点失去局部灵敏度；多维观测则需要进一步确定未知通道作用下的可辨识条件、分支方向和绝对相位基准。本文研究线性接收--解调链中的这一通道可辨识问题，并给出从二维谐波观测恢复全周期相位的精确条件。

本文证明，偏置相位与导频时间波形在余弦传递函数中可以精确分离；在测量窗内参数冻结时，任意线性接收--解调链输出的二维谐波观测必为偏置相位单位圆的仿射像。增益失配、参考相位误差、导频同步串扰和偏置无关的有限消光比只改变椭圆的位置、尺度和方向。满秩观测保证椭圆可逆，直流相位锚点与扫描方向固定其正交规范；在标定参数的有效期内，闭式反演给出全周期相位。该几何与位移干涉测量中的~Heydemann 校正~\cite{heydemann1981,fitzgibbon1999} 相通，本文进一步建立了偏压域中的绝对相位规范和闭环设定点对应关系。主要贡献如下：

\begin{enumerate}
\item \emph{精确观测结构与统一解释}。建立适用于任意周期导频波形和线性接收--解调链的精确仿射定理，给出通道矩阵、确定性偏置与随机噪声的闭式表达，并将特殊点锁定、幅值匹配和谐波比值归结为二维观测的不同低维读出。
\item \emph{全周期辨识与闭式控制}。证明满秩观测与唯一规范固定构成全周期相位可辨识的充要条件，将标定分解为椭圆几何辨识、反射定向和绝对相位规范固定，并由此得到目标点无关的相位反演与积分控制律。
\item \emph{双路径标定与硬件验证}。给出监督回归和纯椭圆加直流规范两种实现。数值实验验证精确闭合、全周期反演和一致闭环增益；真实硬件的五折交错留出~rms 分别为~53.7 和~61.7~mrad，16 点静态响应斜率为~1.052、$R^2=0.986$，静态重复采集与逐~RF 状态重辨识进一步检验采集重复性和静态载荷下的可重标定性。
\end{enumerate}
...
理想光生电流项~$i_{\rm ph}=\rho P_{\rm out}$ 的~Jacobi--Anger 展开为
\begin{equation*}
\begin{aligned}
i_{\rm ph}(t)
={}&\frac{\rho P_0}{2}
\bigl[1+\eta J_0(m)\cos\varphi_b\bigr]\\
&+\eta\rho P_0\cos\varphi_b
\sum_{k=1}^{\infty}J_{2k}(m)\cos(2k\omega t)\\
&-\eta\rho P_0\sin\varphi_b
\sum_{k=0}^{\infty}J_{2k+1}(m)\sin[(2k{+}1)\omega t].
\end{aligned}
\end{equation*}
偶次谐波和直流可变项携带~$\cos\varphi_b$，奇次谐波携带~$\sin\varphi_b$。当~$J_0(m),J_1(m),J_2(m)\neq0$ 时，直流分量提供规范基准，H1/H2 构成二维相位观测，并分别给出零/峰值点与正交点的传统锁定误差。理想极限（$\theta_i{=}0$、正弦~$\xi$、$i_x{=}0$）下，
\begin{equation}
...
正交点锁定驱动~$X\to 0$。其局部增益~$|\mathrm{d}X/\mathrm{d}\varphi_b|=\eta\kappa_2|\sin\varphi_b|$ 在~$\pi/2$ 处取极大，两个正交点由~$\operatorname{sgn}Y$ 判别，过零点对共模功率波动一阶免疫。零/峰值点锁定驱动~$Y\to0$，增益~$\eta\kappa_1$，峰/谷由~$\operatorname{sgn}X$ 判别。为统一表示接收链失配，记~$A=[A_{ij}]$ 为二维通道矩阵、$\bv=(b_X,b_Y)^{\T}$ 为确定性偏置，其完整表达在第~\ref{sec:affine} 节给出。分别在四个特殊点线性化锁定条件，得到
\begin{equation}
\begin{aligned}
...
\begin{theorem}[精确仿射性]
\label{th:affine}
设~$\xi(t)$ 为任意周期导频波形（不必正弦、不必小信号），接收为任意线性链路加线性解调泛函~$\Lc_1,\Lc_2$。记二维锁相观测为~$\zv\triangleq(X,Y)^{\T}$，则存在通道矩阵~$A\in\mathbb{R}^{2\times2}$、确定性偏置~$\bv\in\mathbb{R}^2$ 和随机噪声~$\nv\in\mathbb{R}^2$，使
\begin{equation}
...
\label{prop:ident}
设无噪满周期扫描覆盖~$\mathbb{S}^1$，接收--解调参数在扫描期间不变。由无序轨迹和单次观测唯一确定~$\varphi_b\bmod 2\pi$，当且仅当：(i)~$\operatorname{rank}A=2$；(ii)~辅助信息唯一固定椭圆分解的~$O(2)$ 规范，其中~$O(2)$ 为二维正交群。
\end{proposition}
\begin{proof}
...
\subsection{一致增益闭环}
闭环架构如图~\ref{fig:arch} 所示。取~$e=\operatorname{wrap}_{(-\pi,\pi]}(\hat\varphi_b-\varphi^\ast)$。当仿射模型成立时，解调后的相位误差与真实偏差直接对应，并与目标点位置解耦。配合常数执行斜率~$\partial\varphi_b/\partial V_b=\pi/V_\pi$，同一组控制参数可用于整个偏置周期。本文采用增量积分更新；偏置漂移表现为低频扰动，其残余跟踪误差由环路带宽与漂移速率之比决定。表~\ref{tab:schemes} 中的零静差定义在已标定模型内，对应消除标量读出的死区、分支歧义和标称增益偏差；噪声、有限带宽、执行器饱和与模型漂移进入实际误差预算。传统方案的静差由式~\eqref{eq:staticQ}--\eqref{eq:staticAM} 给出。
...
上电标定后，以独立预检测量局部斜率和尾段动态，并由原始误差序列的标准差、周期--2 幅度、饱和状态和最终绝对误差共同选择~$G$；尾段中心用于评价静态设定点映射，逐周期序列用于评价动态稳定性。
...
实现时有三类参数需要整定。\emph{阈值整定}：$\rho_{\rm th}$ 由稳定标定末尾的残差分布给出，EWMA 与连续~$M$ 周期判决在误触发率和检测延迟之间折中。一次无变化运行和一次阶跃响应验证了判决路径；统计误报率与检出率需要独立重复事件。\emph{重定标激励}：完整椭圆与规范重新辨识要求覆盖充分相位范围。台架采用~121 点满周期扫描；继承旧模型并施加正则约束后，局部微弧可更新局部参数，本文在仿真中检验了~$\pm0.3$~rad 的受限更新。\emph{残差可观测性}：圆残差~$\rho=|\|B(\zv-\hat\bv)\|-1|$ 对径向、中心和椭圆形状变化敏感。规范旋转~$A\mapsto AR$（$R\in O(2)$）保持椭圆集合不变，其相位偏移由直流相位锚点残差、周期参考点或受控激励监测。$\atan$ 可由~CORDIC 或查表实现，参数存储量为六个标量。
...
图~\ref{fig:mzmloop} 给出任意目标点~$\varphi^\ast=1.9$ 处的闭环对比：仿射控制器的~rms 误差为~$12.7$~mrad，H1 单坐标结构消融为~$1315$~mrad，静差主要来自未辨识的仿射失配。图~\ref{fig:sweep} 将比较扩展到目标相位网格，并加入等信息二维基线：该基线使用真值偏置和对角增益，只移除~$A$ 的非对角混合。其残余相位偏差说明，二维观测提供全周期坐标，完整仿射校正进一步辨识非正交通道。统一仿真模型隔离非对角校正的贡献，硬件扫描则检验真实链路中的模型拟合。
...
\section{实验验证}
\label{sec:exp}
本节用四组实验检验二维仿射观测：满周期标定回答模型能否由实测谐波辨识；16 点扫描比较二维反演与单坐标消融；长记录刻画闭环动态；RF 加载和静态重复实验分别检验逐状态重辨识与采集参考的一致性。表~\ref{tab:exp} 汇总各组结果。

\subsection{实验平台与评价协议}
实验平台如图~\ref{fig:exp_mzm} 所示。$1550$~nm、约~$9$~dBm 的分布反馈激光器驱动被测~MZM，调制器输出经~1:9 光耦分为交流导频监测链与~RF 测量链。偏压板以~$64$~kSPS 采样，STM32H523 用~20~ms Goertzel 块提取~1/2~kHz 的~H1/H2，DAC8568 将上位机计算的偏置与导频回写至调制器。DM858E 直接读取~PD 直流输出，给出满周期电压--相位标尺和独立的静态目标评价；50~MHz 加载时，DG922 Pro 驱动射频口，$90\%$ 光口经~PP-10G 接收机送入~FSV30。

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figs/fig_exp_mzm.pdf}
\caption{完整实测平台。主光路为~DFB 激光器~$\rightarrow$ MZM~$\rightarrow$ 1:9 光耦。$10\%$ 端口经~PD、隔直/TIA、ADC 与~STM32 形成交流导频监测链；STM32 完成~Goertzel 谐波提取并通过~USB 串口与~PC 上位机交换~H1/H2 和偏置命令，PC 执行仿射回归、$\atan$ 相位反演与积分更新，DAC8568 叠加偏置和导频后回写~MZM。DM858E（DE4）直接读取~PD 直流值，完成直流标定和真值评价。仅在~50~MHz RF 加载阶段，DG922 Pro 驱动~MZM 射频口，$90\%$ 光口经~PP-10G PIN 前置放大接收机转换为~50~$\Omega$ SMA 电信号后送入~FSV30，测量光输出中的~RF 单音。灰色点线表示~PC 对辅助仪器的控制与数据记录。}
\label{fig:exp_mzm}
\end{figure*}

开环双向慢扫生成~$V_\pi$、$\varphi_0$ 和监督回归所需的相位标签；同批~H1/H2 数据还用于回放第~\ref{sec:calib} 节的纯椭圆加直流规范路径。16 个目标点的静态响应由尾段平均命令回写后的~DMM 读数评价，逐周期尾段序列单独刻画动态。RF 实验在每个功率态重新标定后测试~8 个目标；静态重复实验则在~5 个满周期偏置点比较四种重启条件，每种条件重复~12 次。H1 消融与本文方法共用采集链和执行器，仅将二维相位反演换成单坐标读出。

\begin{table}[!t]
\caption{硬件验证结果汇总。仿真列来自表~\ref{tab:results}。}
\label{tab:exp}
\centering
\scriptsize
...
平均命令静态评价：监督式仿射~rms & 16 点；尾段中心处~DMM 评价 & $12.7$~mrad & $246$~mrad\\
平均命令静态评价：H1 消融~rms & 同上 & --- & $1241$~mrad\\
观测各向异性~$\kappa(A)$\textsuperscript{b} & $m{=}1.2$／$0.092$ & $2.59$ & $45.24$\\
捕获整定时间 & 至~$20$~mrad & $20$--$22$ 周期 & ---\\
漂移检测延迟 & 圆残差触发 & $31$ 周期 & 检测~$6$ 周期\\
重定标后尾段~rms\textsuperscript{c} & 满周期重扫后 & $13.7$~mrad & $396$~mrad\\
长运行诊断：重定标事件\textsuperscript{d} & 连续~$3$~h；周期--2 状态 & --- & 0 次\\
长运行诊断：DMM rms & 连续~$3$~h；60 个稀疏点 & --- & $487$~mrad\\
逐 RF 状态重标定后~rms\textsuperscript{e} & $50$~MHz & --- & 仿射~$192.5$~mrad\\
RF 加载：关~rms & 参考 & --- & $166.1$~mrad\\
静态采集重复性 & 5 点$\times$4 状态$\times$12 次 & --- & H1 相位圆标准差最大~$0.330$~mrad\\
\bottomrule
\end{tabular}
\par\vspace{2pt}
...
\textsuperscript{d}~逐周期解调~rms 为~827~mrad，lag-1 相关系数为~$-0.995$，相邻符号翻转率为~100\%。\quad
\textsuperscript{e}~每个 RF 功率态分别重新标定。\par
注：仿真列对应算法机制，硬件列对应各自标注的实测协议；两者的导频深度、环路速率和标定方法不同。16 个目标点刻画相位覆盖，动态稳定性由逐周期序列单独给出。\par}
\end{table}

\subsection{标定结构与样本外相位反演}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_expcal_mzm.pdf}
\caption{直流与监督式仿射标定。(a) DM858E 双向慢扫及余弦拟合，上/下行~$V_\pi=5.36/5.20$~V，平均~$5.28$~V；(b) 由同一扫描生成相位标签；(c) $A_p=0.15$~V（$m=0.092$）时的观测椭圆，$\kappa(\hat A)=45.24$；(d) 由相位标签直接回归的~$(A,\bv)$ 将拟合样本回拉至单位圆，拟合内中位误差~$27.3$~mrad。纯椭圆回放的中位与~P95 分别为~49.2 和~115.3~mrad。}
\label{fig:expcal}
\end{figure}

DM858E 双向传递曲线给出上、下行~$V_\pi=5.36/5.20$~V，平均~$5.28$~V（图~\ref{fig:expcal}(a)(b)）。以该标尺生成相位标签后，监督回归把观测椭圆回拉至单位圆，拟合内中位误差为~27.3~mrad；同批数据上的纯椭圆回放得到中位~49.2~mrad 和~P95~115.3~mrad（图~\ref{fig:expcal}(c)(d)）。

五折交错留出进一步给出监督路径~27.7/105.6/53.7~mrad 和纯椭圆路径~48.7/114.1/61.7~mrad 的中位/P95/rms。将~$A$ 的非对角项置零后，完整二维与对角二维读出的留出~rms 分别为~53.7 和~60.3~mrad；实测非对角项的~Frobenius 范数占比为~0.85\%，与该台架接近对角的观测链一致。
...
\caption{全周期目标响应与~RF 状态重辨识。(a) 椭圆条件数~$\kappa(\hat A)$ 随导频深度~$m$ 的趋势，$\kappa$ 刻画观测各向异性。(b) 16 个目标的尾段中心静态评价：监督式仿射和~H1 单坐标消融的跨目标~rms 分别为~246 和~1241~mrad。(c) 50~MHz 单音下~H1 衰落及同一数据的~$J_0$ 拟合，等效~$V_\pi^{\rm RF}\approx2.8$~V；(d) 每个~RF 功率态分别重标定后的~8 点跨目标~rms 与~$\kappa(\hat A)$。}
\label{fig:expperf}
\end{figure}

导频深度~$m\approx0.09$--$0.12$ 时，实测~$\kappa(\hat A)\approx45$，弱~H2 决定最小奇异值和相位噪声增益（图~\ref{fig:expperf}(a)）。16 点扫描的尾段平均命令经~DMM 静态评价后，二维仿射反演的跨目标~rms 为~246~mrad，H1 消融为~1241~mrad（图~\ref{fig:expperf}(b)）；宽扫电压--相位映射得到相同排序，对应~342 和~2491~mrad。两种评价下的响应斜率分别为~1.052/1.039，$R^2$ 为~0.986/0.980；15 个相邻步进中~14 个方向一致，16 个点的绝对误差均小于~$\pi/4$，因而覆盖完整相位周期。

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_expdiag_mzm.pdf}
\caption{闭环动态与采集重复性。(a)~3~h 记录中奇偶控制周期相位误差的~61 点移动均值及~DMM 稀疏评价，两个分支对应持续周期--2 状态，逐周期~rms 为~827~mrad、lag-1 相关系数为~$-0.995$。(b)~导频幅度阶跃后，圆残差在~6 个控制周期内触发~121 点重扫，尾段~rms 为~396~mrad。(c)(d)~冻结的~v1.2 静态重复实验：5 个满周期偏置点、4 种重启条件、每种~12 次；H1/H2 相位圆标准差最大值分别为~0.330/29.9~mrad，金虚线给出~20~mrad 重启增量门的尺度和~50~mrad 环境判据。}
\label{fig:expdynrepeat}
\end{figure}

逐周期数据揭示了静态中心以外的闭环动态：6/16 个目标的尾段标准差为~0.716--1.067~rad，合并尾段解调~rms 为~571~mrad。3~h 记录呈持续周期--2 状态，逐周期误差~rms 为~827~mrad、lag-1 相关系数为~$-0.995$（图~\ref{fig:expdynrepeat}(a)）。导频幅度阶跃后，圆残差在~6 个控制周期内触发~121 点重扫，尾段~rms 为~396~mrad（图~\ref{fig:expdynrepeat}(b)）。这些记录把当前台架的动态约束定位到弱轴噪声、控制增益与端到端时延的耦合。

\subsection{RF 状态重辨识与采集重复性}
50~MHz 单音从~RF 关扫至~$+13$~dBm，并在每个功率态重新标定后测试~8 个目标。H1 衰落符合~$J_0(m_{\rm RF})$ 趋势，等效~$V_\pi^{\rm RF}\approx2.8$~V；各状态的~$\kappa(\hat A)$ 约为~45，跨目标~rms 为~62--193~mrad（图~\ref{fig:expperf}(c)(d)）。结果表明，静态~RF 载荷改变后，二维观测可通过逐状态重辨识恢复全周期相位坐标。

图~\ref{fig:expdynrepeat}(c)(d) 将新增的静态重复实验从汇总数字展开为满周期分布。240 次重复产生~480 个正式原始窗，全部通过采样、余量、DMM 括号与调度合同，最大绝对输入为~0.672~V。四种条件下的~H1 参考相位均保持在~0.330~mrad 以内，最大重启增量为~0.287~mrad；H2 的最大圆标准差为~29.9~mrad，低于~50~mrad 环境判据。发生器、采集器及联合重启均未触发预注册的重启异常条件，采集参考的短时重复性由此得到直接验证。
...
\emph{硬件实现。}对已有~H1/H2 双谐波锁相固件，运行时主要增加一次~$2{\times}2$ 乘加和一次~$\atan$，可由~CORDIC 或查表实现。板载化的关键设计量是定点精度、端到端时延和满周期重扫占用时间，而非计算复杂度。

\emph{适用范围。}仿射结构适用于线性接收、线性解调且扫描期间参数近似不变的链路；光电探测/跨阻非线性、偏压相关吸收和直流规范漂移会形成模型外残差。单器件实验验证了监督式与纯椭圆仿射解调，静态重复实验确认采集参考稳定，逐周期记录则把台架动态约束定位到控制增益、端到端时延与弱~H2 噪声的耦合。H1 对照隔离单坐标死区和分支歧义；DLA2C~\cite{hu2026dla2c} 以训练和迭代搜索换取免测弱~H2，本文保留~H2 并通过闭式反演实现显式通道校正。

\emph{推广方向。}纯椭圆加直流规范可直接扩展到无标签硬件闭环，等信息二维对照能够量化非对角校正收益，跨器件测试则检验参数缩放与外部有效性。该框架还可推广到嵌套或多电极调制器；在线设计可用式~\eqref{eq:var} 的最坏相位方差联合优化导频深度与频率，并以相位锚点创新量补足圆残差的规范盲区。

\section{结论}
本文建立了线性接收--解调链下二维谐波观测的精确仿射结构，并将全周期相位可辨识性归结为两个条件：$A$ 满秩，以及绝对相位的~$O(2)$ 规范得到唯一固定。由此形成的椭圆辨识、直流规范和闭式~$\atan$ 反演，把任意点偏压控制写成一次标定后的相位反馈问题。数值实验验证了精确闭合、全周期反演和目标点无关的闭环增益；真实硬件的监督式与纯椭圆路径取得~53.7 和~61.7~mrad 的交错留出~rms，16 点静态目标响应覆盖完整周期，静态重复实验给出最大~0.330~mrad 的~H1 参考相位圆标准差。逐周期实验同时识别出弱轴噪声、控制增益与端到端时延耦合形成的周期--2 动态，为板载实现提供了明确的增益和稳定裕度约束。理论与测量共同说明，二维仿射观测为单~MZM 任意点偏压提供了可辨识、可标定且计算紧凑的控制坐标。

\section*{数据与代码可用性}
补充材料提供本文图表所用的处理后实验数据、数值合同、绘图脚本、采集编排脚本、静态重复协议及独立验证器，可重建全部图表和表中数值；数据字典与文件校验值见随附清单。离线分析与论文图表可由该材料独立复现，硬件重采集还依赖自研板固件、模拟前端原理图和实验室仪器驱动。
