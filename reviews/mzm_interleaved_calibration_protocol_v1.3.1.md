# MZM 局部双向交错标定诊断协议 v1.3.1（冻结弱轴杂散表）

版本：2026-07-18 v1.3.1（A72 后、任何 v1.3.1 实测前冻结）

本版只修订共同 pilot 的 headroom 规划，不改变科学门。A72 的 v1.3 donor 三段已
永久封存：0.09 V 下 segment 1 的一个有效正式窗达到 0.981189 V，超过冻结的
0.95 V 门，故 bundle 如实拒绝。v1.3.1 不重用或只补该段，也不放宽阈值；donor
与 recipient 均改用固定 **0.07 V**，并要求从 donor segment 0 开始完整新 campaign。
0.07 V 在 A44 已实测通过的 0.06/0.08 V 之间；相对于 A57 的 0.08 V 尾值
0.949388 V 和 A72 的 0.09 V 尾值 0.981189 V 提供预先确定的幅度余量，同时比
0.06 V 保留更大的二次谐波。选择规则在任何 v1.3.1 数据采集前冻结，后续不得按
结果改 pilot、挑窗或只重跑失败 segment。
...
- donor 与 recipient 都固定单个 1 kHz、**0.07 V** pilot。该幅度是本版唯一物理
  修订；两阶段使用同一 pilot，禁止依赖跨 pilot 幅度缩放。A44 的相邻 0.06/0.08 V
  全周期正式最大 raw 为 0.51745/0.68471 V；A57 与 A72 的长序列尾值说明短程量程
  扫描不能替代完整 campaign 的 0.95 V 门，因此本版仍逐窗检查且任何超限都失败。
- acquisition frequencies 固定为 1000/2000 Hz；每个 segment 在 conditioning 前
  ...
  0.07 V pilot 与恰好两个 acquisition frequencies。
...
segment 独立通过且全局 corrected bundle 的全部门通过，v1.3.1 才可
