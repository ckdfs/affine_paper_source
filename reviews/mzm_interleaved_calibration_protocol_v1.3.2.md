# MZM 局部双向交错标定诊断协议 v1.3.2（冻结弱轴杂散表）

版本：2026-07-18 v1.3.2（A79 后、任何 v1.3.2 实测前冻结）

本版只修订共同 pilot 与预先固定的 formal averaging，不改变科学门。A79 的 v1.3.1
donor 三段已永久封存：0.07 V 下 segment 1 的一个有效正式窗出现 1276 个正 rail
样本，故 bundle 如实拒绝；科学门本身全部通过。v1.3.2 不重用或只补该段，也不放宽
阈值。donor 与 recipient 均改用固定 **0.04 V**，donor formal windows 从 8 增至
**16**、recipient 从 4 增至 **8**，并要求从 donor segment 0 开始完整新 campaign。

0.04 V 相对 A75 的 0.07 V 最大 RAW 0.919598 V 按 pilot 线性缩放对应约 0.526 V，
也低于 A44 的 0.06 V 实测正式最大值；该选择给稀有幅度突发留出显著余量。H2 随
pilot 近似二次下降，因此两阶段 formal 窗数同时加倍，以降低半样本和 recipient 均值
噪声；DMM 仍固定 8+8 读数。选择和平均长度在任何 v1.3.2 数据采集前冻结，后续不得
按结果改 pilot、挑窗或只重跑失败 segment。
...
- donor 与 recipient 都固定单个 1 kHz、**0.04 V** pilot。两阶段使用同一 pilot，
  禁止依赖跨 pilot 幅度缩放。短程量程扫描和线性缩放不能替代完整 campaign 的
  0.95 V 门，因此本版仍逐窗检查且任何超限都失败。
- acquisition frequencies 固定为 1000/2000 Hz；每个 segment 在 conditioning 前
  ...
  0.04 V pilot 与恰好两个 acquisition frequencies。
...
（pre[0..7]），八/十六个正式窗后再连续读取 8 次（post[0..7]）。每次读数都保存
...
`16 x acq run 16 blocks` 正式窗。十六窗按 index 偶/奇分为两个预先固定的独立半样本：

```text
A = windows {0,2,4,6,8,10,12,14};  B = windows {1,3,5,7,9,11,13,15}
```
...
recipient 每 observation 固定 `8 x acq run 16 blocks`。原始 `X_raw/Y`、全部 I/Q
...
segment 独立通过且全局 corrected bundle 的全部门通过，v1.3.2 才可
...
6. 在首个 discard 已落盘后失败、在 DMM reads 中途失败、cleanup failure 均保留
   partial 审计文件并令 manifest failed；validator 拒绝旧 v1.2/v1.3 bundle 冒充
   v1.3.2。
