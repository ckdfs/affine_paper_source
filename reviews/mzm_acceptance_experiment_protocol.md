...
目的：补齐 `paper_mzm_zh.tex` 当前无法由既有数据解决的三项证据缺口：
无相位标签的椭圆加直流规范闭环、同信息二维强基线、独立重复与成组统计。
本协议在采集新数据前固定控制器、随机化、评价口径和通过条件，避免结果导向的
阈值选择。
...
控制器使用的直流序列只负责规范固定，不是独立真值。论文接收门要求另一路不向
...
工具链烟雾测试只能写入 `build/exp_sim/`，且永远不能通过论文接收门：
