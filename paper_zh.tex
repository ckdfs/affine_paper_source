...
判据：仿射框架在任意目标点的闭环静差应与标定残差同量级、且不随目标相位系统性放大，并显著优于幅值匹配/谐波比值基线；受导频深度限制，各项绝对指标按式~\eqref{eq:var} 的~$\kappa(m)$ 趋势整体高于~$m{=}1.2$ 仿真（见下文实测）。
...
\emph{DPMZM 扩展（计划/未来工作）。}本平台当前仅含单~MZM；DPMZM 的三轴验证留待接入~DPMZM 器件后进行，硬件侧已就绪（板~DAC8568 八路偏置、\texttt{gen} 最多三音导频、\texttt{acq} 可取六谐波与三个~IMD 通道；计划链路见图~\ref{fig:exp_dpmzm}）。计划阶段：（5）子轴~$4\pi$ 与~Klein——子~MZM 扫~$4V_\pi$，验证~$\varphi$ 与~$\varphi{+}2\pi$ 在输出上可区分（图~\ref{fig:exp6}~$\leftrightarrow$~图~\ref{fig:torus}）；（6）可观测性恢复——标准~QPSK 点附近测父轴环路增益与解调方差，对比六谐波通道与加入~IMD 后的九通道（图~\ref{fig:exp5}~$\leftrightarrow$~图~\ref{fig:obs}）；（7）任意点三轴锁定——对比三独立环的失锁率与静差（图~\ref{fig:exp4}~$\leftrightarrow$~图~\ref{fig:dploop}）。判据：IMD 通道确能在标准点附近恢复父偏置可观测性，任意点三轴~rms 显著优于三独立环基线。
