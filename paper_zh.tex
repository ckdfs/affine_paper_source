...
\section{实验验证计划（待实测）}
\label{sec:exp}
本节给出待实测验证计划。当前稿件中的实验数据尚未填入，表~\ref{tab:exp} 和图~\ref{fig:exp_mzm}--图~\ref{fig:exp6} 用于固定测量链路、测试项目、指标和与数值仿真的逐项对应关系。后续实测完成后，应以实测值替代表中“待测”项，并用实测曲线替换占位图框。

\emph{测量链路。}控制环运行在上位机上（单~MZM 见图~\ref{fig:exp_mzm}，DPMZM 见图~\ref{fig:exp_dpmzm}）。TLS1000 激光器（$1550$~nm、$+10$~dBm）入射被测调制器，入射光功率约~$0$~dBm；器件插入损耗约~$6$~dB、消光比~$30$~dB、$V_\pi\approx5$~V（待实测）。输出经~1:9 耦合器，$10\%$ 支路送~Agiltron PP-10G 放大探测器（响应度~$0.88$~A/W、跨阻~$500\,\Omega$、带宽~$12$~GHz、输入噪声~$16.5$~pA/$\sqrt{\rm Hz}$），其电输出进入~Siglent SDS824X HD 示波器（$200$~MHz、$2$~GSa/s、$12$~bit）。上位机从示波器~FFT 读取各谐波与互调频点的功率（即观测向量~$\zv$），完成仿射反演与~PI 更新，再由~Rigol DG922 Pro 任意波形发生器（$200$~MHz、$16$~bit、最大~$20$~Vpp）产生偏置与导频写回调制器；DG922 Pro 为双通道，DPMZM 的三路偏置需两台或多通道源。导频取~$400$~mVpp/音，深度~$m=\pi A_p/V_\pi\approx0.13$；由式~\eqref{eq:var}，$\kappa(A)\approx30$，观测椭圆较扁，须以足够的~FFT 平均换取~H2 与互调通道的信噪比。三音频率置于~PP-10G 通带内（数十~kHz 至数~MHz）、按互调不碰撞规划（沿用仿真的~$1{:}1.37{:}1.93$ 比例），谱线远在示波器带宽内。受示波器采集、FFT 平均与仪器通信限制，PC 环路速率为~Hz 量级，但仍远高于偏置漂移（毫弧度每分钟），足以跟踪。PP-10G 为交流耦合（低频截止约~$30$~kHz），输出不含光功率直流项，故规范固定所需的直流由另一路慢速探测器获取。框架验证时~RF 关断（对应式~\eqref{eq:dpmzm} 模型），加载~RF 数据则可考察鲁棒性。面向嵌入式的小型化（第~\ref{sec:algo} 节估计每周期~$<10^3$ 乘加）可在后续移入~Cortex-M 级固件。

\begin{figure}[!t]
\centering
\includegraphics[width=0.95\textwidth]{figs/fig_exp_mzm.pdf}
\caption{单~MZM 的~PC 仪器闭环链路（示意）。TLS1000 激光入射~MZM，输出由~1:9 耦合器分出~$10\%$ 至~PP-10G 探测器，电输出进入~SDS824X HD 示波器。上位机由示波器~FFT 读取观测向量~$\zv$、完成仿射反演与~PI，再经一台~DG922 Pro 产生偏置与单音导频（$400$~mVpp）写回，构成闭环。规范固定所需直流由另一路慢速探测器获取（图中从略）；RF 关断。}
\label{fig:exp_mzm}
\end{figure}
...
\caption{DPMZM 的~PC 仪器闭环链路（示意）。与图~\ref{fig:exp_mzm} 同构，区别在于三路偏置与三音导频（$\omega_1,\omega_2,\omega_3$），由\emph{两台}~DG922 Pro（共六通道）产生；示波器~FFT 同时读取六谐波与三个互调通道。}
\label{fig:exp_dpmzm}
\end{figure}

\emph{相位真值与精度评定。}任意点锁定精度的实验评定需要一个独立于控制环的偏置相位参考。计划用开环慢扫描、经前述慢速探测器记录整条传递曲线~$P(V_b)$：由相邻极值定出~$V_\pi$ 与~$\varphi_0$，从而把任一直流偏压标定为相位~$\hat\varphi(V_b)$，作为锁定误差~$|\varphi_{\rm lock}-\varphi^\ast|$ 的基准。单~MZM 还可用直流功率极值提供绝对相位锚点；DPMZM 则在固定父偏置下逐子轴重复该扫描。该慢速支路与~PP-10G 高速支路同源，故评定的是相对一致性而非绝对光功率定标。

\emph{分阶段流程与判据。}测试按以下阶段推进，每阶段对照一张仿真图：
（0）平台与~$V_\pi$ 标定；
（1）单~MZM 标定——满周期慢扫、椭圆拟合、规范固定、自检残差（图~\ref{fig:exp1}~$\leftrightarrow$~图~\ref{fig:ellipse}）；
（2）单~MZM 任意点锁定——$\varphi^\ast$ 取~$\ge16$ 点逐点锁定，用相位基准读~rms 与静差，对比~H1 幅值匹配基线（图~\ref{fig:exp2}~$\leftrightarrow$~图~\ref{fig:mzmloop}、\ref{fig:sweep}）；
（3）噪声与导频深度——测相位残差随~$\varphi$ 的分布并扫~$m$，核对~$\kappa(A)$ 趋势（$\leftrightarrow$~图~\ref{fig:bessel}）；
（4）漂移与重定标——温箱慢漂叠加人为突变（阶跃改变激光功率或导频幅度），记检测延迟、恢复~rms 与重定标次数，并作~$24$~h 连续运行（图~\ref{fig:exp3}~$\leftrightarrow$~图~\ref{fig:recal}）；
（5）DPMZM 子轴~$4\pi$ 与~Klein——子~MZM 扫~$4V_\pi$，验证~$\varphi$ 与~$\varphi{+}2\pi$ 在输出上可区分（图~\ref{fig:exp6}~$\leftrightarrow$~图~\ref{fig:torus}）；
（6）DPMZM 可观测性恢复——标准~QPSK 点附近测父轴环路增益与解调方差，对比六谐波通道与加入~IMD 后的九通道（图~\ref{fig:exp5}~$\leftrightarrow$~图~\ref{fig:obs}）；
（7）DPMZM 任意点三轴锁定——对比三独立环的失锁率与静差（图~\ref{fig:exp4}~$\leftrightarrow$~图~\ref{fig:dploop}）。
...
噪声地板~$\kappa(A)$ & $2.59$（$m{=}1.2$） & 待测（$m{\approx}0.13$：$\kappa{\approx}30$）\\
...
\section{结论}
本文从偏置相位与导频时间波形的因子分解出发，证明 MZM 的谐波解调观测是单位圆的精确仿射像，并将该结构推广到 DPMZM 的~12 维三角特征映射。基于这一模型，特殊点锁定及其静差可解释为仿射观测的低维读出；任意点控制则可通过标定、规范固定和相位反演实现，闭环增益不再依赖目标点位置。对 DPMZM，半角结构给出~$4\pi$ 子轴周期、Klein 四元群等价和 IMD 通道可观测性的秩条件解释。数值验证表明，在本文仿真条件下，仿射控制器可达到毫弧度量级残差，并相对于传统多环基线降低任意点静差。该框架的主要边界是接收链非线性、偏压相关吸收和实测平台中的持续激励问题；这些因素需要通过残差监测、重定标和后续实验验证进一步量化。把同一仿射辨识视角推广到嵌套与偏振复用器件、并将已知非线性纳入半参数模型，是值得继续的方向。
