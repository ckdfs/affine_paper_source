#!/usr/bin/env python3
"""Read-only pure-contract fault matrix for frozen MZM interleaved v1.3.2 bundles."""
...
def _expect_subprocess_failure(command, label):
    result = subprocess.run(command, text=True, capture_output=True)
    if result.returncode != 0:
        return True
    raise AssertionError(f"fault subprocess was accepted: {label}")
...
def main():
    ...
    _replay("validate_mzm_interleaved_v132_bundle.py", donor)
    _replay("validate_mzm_interleaved_v132_bundle.py", recipient)

    table = _table(donor / "spur_correction.npz")
    protocol = json.loads((recipient / "protocol.json").read_text())
    donor_protocol = json.loads((donor / "protocol.json").read_text())
    ...
    segment_validator = str(Path(__file__).with_name(
        "validate_mzm_interleaved_v132_segment.py"))
    with tempfile.TemporaryDirectory(prefix="mzm-v132-schedule-") as tmp:
        tampered = Path(tmp) / "recipient_segment"
        source_segment = Path(protocol["source_segments"][0]["path"])
        shutil.copytree(source_segment, tampered)
        segment_protocol = json.loads((tampered / "protocol.json").read_text())
        segment_protocol["segment_schedule"][0]["grid_index"] = (
            int(segment_protocol["segment_schedule"][0]["grid_index"]) + 1)
        acquire._write_json(str(tampered / "protocol.json"), segment_protocol)
        checksums = json.loads((tampered / "checksums.json").read_text())
        checksums["protocol.json"] = acquire._sha256(str(tampered / "protocol.json"))
        acquire._write_json(str(tampered / "checksums.json"), checksums)
        pure["recipient_schedule_tamper_rejected"] = _expect_subprocess_failure(
            [sys.executable, segment_validator, "--replay-dir", str(tampered)],
            "recipient schedule tamper")

    aggregate = str(Path(__file__).with_name(
        "analyze_mzm_interleaved_v132_segments.py"))
    recipient_segments = [str(value["path"])
                          for value in protocol["source_segments"]]
    aggregate_prefix = [
        sys.executable, aggregate, "--stage", "recipient", "--donor-dir",
        str(donor), "--run-id"]
    aggregate_cases = {
        "recipient_missing_segment_rejected": recipient_segments[:2],
        "recipient_duplicate_segment_rejected": [
            recipient_segments[0], recipient_segments[1], recipient_segments[1]],
        "recipient_reordered_segments_rejected": [
            recipient_segments[1], recipient_segments[0], recipient_segments[2]],
    }
    for name, paths in aggregate_cases.items():
        run_id = f"{output.stem}_{name}"
        pure[name] = _expect_subprocess_failure(
            aggregate_prefix + [run_id] + paths, name)

    old_validator = str(Path(__file__).with_name(
        "validate_mzm_interleaved_v131_bundle.py"))
    pure["old_v131_validator_rejected"] = _expect_subprocess_failure(
        [sys.executable, old_validator, "--replay-dir", str(recipient)],
        "old v1.3.1 validator")
    ...
    report = dict(
        ...
        donor_source_segments=len(donor_protocol.get("source_segments", [])),
