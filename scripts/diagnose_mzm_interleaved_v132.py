#!/usr/bin/env python3
"""Acquire one preregistered MZM interleaved-v1.3.2 donor/recipient segment."""
...
def main():
    ...
    if not args.sim and not args.i_understand_this_writes_real_hardware:
        raise RuntimeError("real v1.3.2 segment requires explicit hardware acknowledgement")
    ...
    family = ("interleaved_spur_calibration" if args.stage == "donor"
              else "interleaved_calibration_v132")
    ...
    try:
        ...
        repo_commit = "unknown"
    source_paths = {
        "diagnose_mzm_interleaved_v132.py": os.path.abspath(__file__),
        "mzm_interleaved_v132_truth.py": os.path.abspath(v13.__file__),
        "mzm_interleaved_v132_contract.py": os.path.abspath(contract.__file__),
        ...
        "validate_mzm_interleaved_v132_segment.py": os.path.join(
            ec.REPO, "scripts", "validate_mzm_interleaved_v132_segment.py"),
        "analyze_mzm_interleaved_v132_segments.py": os.path.join(
            ec.REPO, "scripts", "analyze_mzm_interleaved_v132_segments.py"),
        "validate_mzm_interleaved_v132_bundle.py": os.path.join(
            ec.REPO, "scripts", "validate_mzm_interleaved_v132_bundle.py"),
        "protocol.md": os.path.join(
            ec.REPO, "reviews", "mzm_interleaved_calibration_protocol_v1.3.2.md")}
    protocol = dict(
    ...
    # v1.3.2 donor segments contain 16 formal windows per observation; keep
    # independently simulated sessions strictly non-overlapping as required by
    # the same aggregate contract used for real sessions.
    sim_clock = 1_800_600_000.0 + 2400.0 * args.segment_index
    invalid_used = False
