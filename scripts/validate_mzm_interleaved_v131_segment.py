#!/usr/bin/env python3
"""Read-only full replay for one MZM interleaved-v1.3.1 segment."""
...
def main():
    ...
    if missing:
        raise RuntimeError(f"missing v1.3.1 segment files: {missing}")
    checksums = json.loads((root / "checksums.json").read_text())
    if set(checksums) != required - {"checksums.json"}:
        raise RuntimeError("v1.3.1 segment checksum file set differs")
    for name, digest in checksums.items():
        if legacy._sha256(root / name) != digest:
            raise RuntimeError(f"v1.3.1 segment checksum mismatch: {name}")
    ...
    if protocol.get("protocol_version") != expected_version:
        raise RuntimeError("v1.3.1 segment protocol/stage differs")
    segment = int(protocol.get("segment_index", -1))
    if segment not in range(3):
        raise RuntimeError("v1.3.1 segment index differs")
    if manifest.get("status") != "complete" or manifest.get("failure") is not None:
        raise RuntimeError("v1.3.1 segment manifest is not complete")
    ...
    if (protocol.get("schedule_sha256") != v13.schedule_sha256(schedule) or
        ...
        raise RuntimeError("v1.3.1 segment frozen schedule differs")
    source_hashes = protocol.get("source_sha256", {})
    repo = Path(__file__).resolve().parents[1]
    source_files = {
        "diagnose_mzm_interleaved_v131.py": Path(__file__).with_name(
            "diagnose_mzm_interleaved_v131.py"),
        "mzm_interleaved_v131_truth.py": Path(v13.__file__).resolve(),
        "mzm_interleaved_v131_contract.py": Path(contract.__file__).resolve(),
        ...
        "validate_mzm_interleaved_v131_segment.py": Path(__file__).resolve(),
        "analyze_mzm_interleaved_v131_segments.py": Path(__file__).with_name(
            "analyze_mzm_interleaved_v131_segments.py"),
        "validate_mzm_interleaved_v131_bundle.py": Path(__file__).with_name(
            "validate_mzm_interleaved_v131_bundle.py"),
        "protocol.md": repo / "reviews" /
            "mzm_interleaved_calibration_protocol_v1.3.1.md",
    }
    if not isinstance(source_hashes, dict) or set(source_hashes) != set(source_files):
        raise RuntimeError("v1.3.1 segment frozen source file set differs")
    for name, path in source_files.items():
        if legacy._sha256(path) != source_hashes[name]:
            raise RuntimeError(f"v1.3.1 segment source hash differs: {name}")
    ...
    if (not summary.get("complete") or summary.get("analysis") != recorded or
        ...
        raise RuntimeError("v1.3.1 segment summary/manifest contract differs")
    if not protocol.get("simulated", False):
        ...
        if (str(final.get("State", "")).upper() != "IDLE" or
            ...
            raise RuntimeError("v1.3.1 segment final hardware state is unsafe")
