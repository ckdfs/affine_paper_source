#!/usr/bin/env python3
"""Aggregate three MZM interleaved-v1.3 donor or recipient segments."""
...
import diagnose_mzm_interleaved_v13 as acquire
...
def _load_segment(root):
    ...
    result = subprocess.run(
        [sys.executable, str(Path(__file__).with_name(
            "validate_mzm_interleaved_v13_segment.py")),
         "--replay-dir", str(root)], text=True, capture_output=True)
    if result.returncode != 0:
        raise RuntimeError(f"v1.3 source segment replay failed: {root}\n{result.stderr}")
...
def main():
    ...
    if [int(value["protocol"]["segment_index"]) for value in segments] != [0, 1, 2]:
        raise RuntimeError("v1.3 source segment order differs")
    if any(value["protocol"]["stage"] != args.stage for value in segments):
        raise RuntimeError("v1.3 source segment stage differs")
    hashes = [value["protocol"]["source_sha256"] for value in segments]
    if any(value != hashes[0] for value in hashes[1:]):
        raise RuntimeError("v1.3 source hashes differ across segments")
    ...
    for value in segments[1:]:
        for key in ("device_id", "firmware_rev", "instrument_ids"):
            if value["protocol"]["metadata"].get(key) != reference.get(key):
                raise RuntimeError("v1.3 source hardware metadata differ")
    previous_end = None
    for value in segments:
        ...
        if previous_end is not None and previous_end >= start:
            raise RuntimeError("v1.3 source segment acquisition times overlap")
        previous_end = end
