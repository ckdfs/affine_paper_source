#!/usr/bin/env python3
"""Acquire one preregistered MZM interleaved-v1.3 donor/recipient segment."""
...
def main():
    ...
    if not args.sim and not args.i_understand_this_writes_real_hardware:
        raise RuntimeError("real v1.3 segment requires explicit hardware acknowledgement")
    ...
    family = ("interleaved_spur_calibration" if args.stage == "donor"
              else "interleaved_calibration_v13")
    ...
    try:
        ...
        repo_commit = "unknown"
    source_paths = {
        "diagnose_mzm_interleaved_v13.py": os.path.abspath(__file__),
        "mzm_interleaved_v13_truth.py": os.path.abspath(v13.__file__),
        "mzm_interleaved_v13_contract.py": os.path.abspath(contract.__file__),
        ...
        "validate_mzm_interleaved_v13_segment.py": os.path.join(
            ec.REPO, "scripts", "validate_mzm_interleaved_v13_segment.py"),
        "analyze_mzm_interleaved_v13_segments.py": os.path.join(
            ec.REPO, "scripts", "analyze_mzm_interleaved_v13_segments.py"),
        "validate_mzm_interleaved_v13_bundle.py": os.path.join(
            ec.REPO, "scripts", "validate_mzm_interleaved_v13_bundle.py"),
        "protocol.md": os.path.join(
            ec.REPO, "reviews", "mzm_interleaved_calibration_protocol_v1.3.md")}
    protocol = dict(
            ...
            try:
                ...
                for item in records:
                    ...
                    if args.sim:
                        ...
                        advance(v13.BIAS_SETTLE_S)
                    if args.sim:
                        response = "SIM pilots: 1 f=1000.0Hz amp=0.0900V freqs: 2 f=1000.0Hz f=2000.0Hz"
                    else:
                        board.gen_reset()
                    ...
                    checks = dict(
                        ...
                        amplitude_pass="amp=0.0900V" in response,
